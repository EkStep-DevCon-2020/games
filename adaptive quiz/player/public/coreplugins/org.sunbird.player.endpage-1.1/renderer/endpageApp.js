...
endPage.controller("endPageController", function($scope, $rootScope, $state,$element, $stateParams) {
    ...
    $scope.userName = window.getUrlParameter('userName')
    ...
    $scope.getTotalScore = function(id) {
        ...
        if (!_.isEmpty(teleEvents) && !_.isUndefined(teleEvents.assess)) {
            ...
            if (window.adaptiveQuestion.earnedScore){
                totalScore = window.adaptiveQuestion.earnedScore;
                maxScore = window.adaptiveQuestion.totalScore;
            }
    ...
    $scope.abc = function() {
        ...
        var percentage = ($scope.userScore * 100) / $scope.totalScore
        if (percentage >= 70) {
            returnData = true
        }
        return returnData
